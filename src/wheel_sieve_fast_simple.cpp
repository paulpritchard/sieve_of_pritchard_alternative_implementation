...
/* simple high-performance implementation using a simple array of integers and a bit array (using bytes for speed) */
/* 2 <= N <= 1000000000 */
...
void Delete (uint32_t w[], uint32_t length, uint32_t p, bool d[], uint32_t &imaxf) {
    ...
    #if LOGGING
    printf("After Delete(%d): (partial)", p); check(w, 0, imaxf, d);
    #endif
}
...
void Compress(uint32_t w[], bool d[], uint32_t to, uint32_t &w_end) {
    ...
    #if LOGGING
    printf("After Compress(%d):", to); check(w, 0, w_end, d);
    #endif
}
...
int main (int argc, char *argw[]) {
    ...
    int stop_s=clock();
    printf("%d primes up to %lu found in %.3f ms\n", nrPrimes, (unsigned long)N, (stop_s-start_s)*1E3/double(CLOCKS_PER_SEC));
